...
**CPP combined-benefit ceiling — verified value, unverified rule (§13.2a).** `cppCombinedMax` = $1,531.56/month ($18,378.72/year), ESDC's published "Combined survivor's and retirement pension (at age 65)" maximum — **VERIFIED, `verifiedDate: 2026-08-21`**. The **rule for applying it** in `cppSurvivorBenefit` is **APPROXIMATE**: the age-65 ceiling is subtracted against the survivor's own pension *as actually received* (after `cppFactor`), so a survivor who deferred to 70 can be reduced to zero. Service Canada's consumer pages do not state whether the ceiling adjusts for the survivor's claiming age, and they point at the *retirement* maximum while ESDC's table publishes a higher *combined* maximum. Behaviour is deliberately unchanged; reading (a) is the most conservative for the client. Full statement of the three candidate readings and the resolution path: `docs/AGENT-STATUS.md`, OPEN entry of 2026-08-21. This is a textbook §13.2a case — **a verified constant inside an unverified rule** — and the two statuses must be surfaced separately.
